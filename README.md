...
`sigpy/sigpy.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `sigpy.py` in directory called
`sigpy`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import sigpy as sb` in an
interactive Python session, the classes and functions defined inside of the
`sigpy.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the sigpy module, the main classes defined at the bottom of
...
Either way, you can create a `sigpy/data` folder in which you can
...
(typically, something like our `sigpy/tests/test_sigpy.py`). Within each
...
have one such test in `sigpy/tests/test_sigpy.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/sigpy`),
and type:
...
will see a message such as:
    ...
    sigpy/tests/test_sigpy.py:49: AssertionError
...
To document `sigpy` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/sigpy/blob/master/doc/_static/RTD-advanced-conf.png)

 http://sigpy.readthedocs.org/en/latest/
...
A `sigpy/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/sigpy) for the software. This
...
`sigpy` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `sigpy`, we use the
...
[here](https://github.com/uwescience/sigpy/blob/master/scripts/Figure1.ipynb).
...
`sigpy/sigpy.py` to be called `smallish/smallish.py`

	git mv sigpy smallish
	git mv smallish/sigpy.py smallish/smallish.py
	git mv smallish/tests/test_sigpy.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `sigpy` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `sigpy` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `sigpy` in them. Type
the following to see where all these files are:

	git grep sigpy

You can replace `sigpy` for `smallish` quickly with:

	git grep -l 'sigpy' | xargs sed -i 's/sigpy/smallish/g'
...
Edit all the mentions of `sigpy` in the `sigpy/__init__.py` file, and in
the `sigpy/version.py` file as well.
